...
const Navbar = ({activeScrollProp}) => {
  ...
  return (
    <div className={`${pathname === '/whitepaper' || pathname === '/learn-more' ? 'hidden' : 'flex'} ${activeScrollProp ? 'md:w-[70%] w-[80%] top-5 shadow-md' : 'md:w-full w-[95%] top-0'} fixed -translate-x-1/2 left-1/2 justify-center flex-col rounded-3xl z-40 bg-white`}>
      ...
      <Link href='/'>
        <Image src={minenuLogo} width={1000} height={1000} alt='minenu' className='md:w-15 w-12' />
      </Link>
        <div className={`flex justify-between items-center ${activeScrollProp ? 'w-[80%]' : 'md:w-[50%]'} transition-w duration-200`}>
            <ul className="md:flex space-x-5 items-center mx-auto hidden">
              ...
              <li><Link href='/tokenomics' className="no-underline text-[#191919] text-sm hover:bg-white hover:shadow rounded-xl py-1 px-2">Tokenomics</Link></li>
              <li><Link href='/roadmap' className="no-underline text-[#191919] text-sm hover:bg-white hover:shadow rounded-xl py-1 px-2">Roadmap</Link></li>
            </ul>
        </div>
        <div className={`flex space-x-5`}>
            ...
            <Menu onClick={() => setActiveNav(!activeNav)} size={40} className='md:hidden text-black' />
        </div>
      </div>

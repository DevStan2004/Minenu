...
const Hero = () => {
    ...
    // Add state to track if the screen is large enough for the animation
    ...
    useEffect(() => {
        // Function to check and set the desktop state
    ...
    useGSAP(() => {
        // 🎯 CRITICAL: Only run the animation setup if isDesktop is true
        if (!isDesktop) {
            // Optional: Ensure all targets are in their final, non-split state
            gsap.set('.title', { clearProps: 'all' });
            return;
        }

        const titles = gsap.utils.toArray('.title');
        // This is where the text-split animation is created
        const textSplit = new SplitText('.title', {type: 'chars, words'});
        
        // Initial state for image (can run regardless of breakpoint, but we keep it here for simplicity)
        ...
        mainTimeline.to('.stake', {
            ...
            duration: 10,
            ease: 'expo.out'
        }, '<').to(textSplit.chars, { // This is the text-split animation target
            y: 100, 
            duration: 3,
            stagger: 0.06,
            ease: 'expo.out'
        ...
        return () => {
            // Note: SplitText has a revert method, but for this conditional setup, 
            // the `if (!isDesktop)` block above handles the removal of the effects.
            // When isDesktop turns false, a new useGSAP runs, and the old timeline is killed.
            textSplit.revert();
            mainTimeline.kill();
        }
    }, {
        scope: containerRef, 
        // 🎯 CRITICAL: Re-run the animation logic when isDesktop changes

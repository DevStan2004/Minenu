...
const Hero = () => {
    ...
    useGSAP(() => {
        ...
        if (isDesktop) {
            ...
            textSplit = new SplitText('.title', {type: 'chars, words'});
            ...
            // Always clear SplitText DOM and GSAP props on mobile
            gsap.set('.title', { clearProps: 'all' });
        }

        // Always cleanup SplitText and timeline, regardless of breakpoint
        return () => {
            // Try to revert SplitText if it was created
            if (textSplit && typeof textSplit.revert === 'function') {
                textSplit.revert();
            } else {
                // Defensive: if SplitText was not created, still clear props
                gsap.set('.title', { clearProps: 'all' });
            }
            // Kill timeline if it was created
